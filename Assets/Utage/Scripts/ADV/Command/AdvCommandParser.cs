	...
	public static class AdvCommandParser
	{
		...
		static public AdvCommand CreateCommand(StringGridRow row, AdvSettingDataManager dataManager)
		{
			...
			if (command == null)
			{
				...
				{
                    //記述ミス
                    Debug.LogWarning(row.ToErrorString(LanguageAdvErrorMsg.LocalizeTextFormat(AdvErrorMsg.CommandParseNull)));
                    Profiler.EndSample();
					Profiler.EndSample();
...
//		public const string IdObjectFadeOut = "ObjectFadeOut";		//オブジェクトに対してのフェードアウト
		...
		public const string IdSendMessageToSender = "SendMessageToSender";
		...
		public const string IdSendCharacterCommand = "CharacterCommand";    //指定の名前のオブジェクトにBroadcastMessage処理
		...
		public const string IdCustomCommand = "CustomCommand";
...
*/
				...
				case IdSendMessageToSender:
					return new AdvCommandSendMessageToSender(row);
				case IdBroadcastMessageByName:
					return new AdvCommandBroadcastMessageByName(row);
				case IdSendCharacterCommand:
					return new AdvCommandCharacterHH(row);
