...
namespace Utage
{
	...
	public class AdvPage : MonoBehaviour
	{
		...
		private float currentCharTimeCost;

		public float CurrentTextDuration 
		{
            get
            {
				float autoPageWaitTime = 0;
                //if (!(Engine.Config.IsAutoBrPage && Engine.SoundManager.IsPlayingVoice()))
                //	autoPageWaitTime = Engine.Config.AutoPageWaitTime;

                if (Engine.Config.IsAutoBrPage)
					autoPageWaitTime = Engine.Config.AutoPageWaitTime;

				return CurrentTextLengthMax * Engine.Config.GetTimeSendChar(CheckReadPage()) + autoPageWaitTime;
		...
		public void BeginPage(AdvScenarioPageData currentPageData)
		{
			...
			{
                for (int i = 0; i < currentPageData.TextDataList.Count; i++)
                {
                    for (int j = 0; j < currentPageData.TextDataList[i].RowData.Strings.Length; j++)
                    {
						//Debug.Log("***" + currentPageData.TextDataList[i].RowData.Strings[j]);
					}
		...
		public void SetCurrentTextLength(int len)
        {
		...
		void UpdateSendChar()
		{
			...
			else
			{
				...
				SendChar(timeCharSend , out currentCharTimeCost);
				//Debug.LogError("total time: " + CurrentTextLengthMax * timeCharSend);
		...
		void UpdateWaitInput()
		{
			...
			{
				//Debug.LogError("IsInputSendMessage()");
				//スキップではなく入力でのみトリガー発生
				if (isInputSendMessage)
				{
		...
		void SendChar(float timeCharSend , out float _calculatedCharTimeCost)
		{
			if (timeCharSend <= 0)
			{
				if (IsNoWaitAllText)
				{
					CurrentTextLength = CurrentTextLengthMax;
					_calculatedCharTimeCost = timeCharSend;
					return;
				}
				else
				{
