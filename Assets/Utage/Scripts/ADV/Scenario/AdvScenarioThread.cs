...
namespace Utage
{
	...
	{
		public UnityEvent OnSendLastText;
		//スレッド名
		...
		IEnumerator CoStartScenario(string label, int page, AdvCommand returnToCommand, bool skipPageHeaer)
		{
			...
			while (currentLabelData != null)
			{
				...
				{
                    if (page == currentLabelData.PageNumWithText - 1)
                    {
						OnSendLastText?.Invoke();
                    }
					//int textCount = currentPageData.TextDataList.Count;
					//for (int i = 0; i < textCount; i++)
     //               {
					//	AdvCommandText text = currentPageData.TextDataList[i];
					//	if (text.RowData != null && text.RowData.Grid != null)
					//	{
					//		string t = text.ParseCellLocalizedText();
					//		Debug.Log($"{page} / {t}");
     //                   }
					//}
					//プリロードを更新
