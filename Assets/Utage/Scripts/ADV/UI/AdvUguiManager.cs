...
namespace Utage
{
	...
	public class AdvUguiManager : AdvUiManager
	{
		...
		protected virtual void Update()
		{
			...
			if (Input.GetKeyDown(KeyCode.Return) /*|| HonoHime.InputSystem.InputManager.GetButtonStarted(HonoHime.InputSystem.InputManager.Button.Submit)*/)
            {
                //if (HHUtageTLStatic.UtageTLInputOverride)
                //{
     //               if (inputLocker) 
					//{
					//	Debug.Log("inputLocker");
					//	IsInput = false;
     //               }
     //               else
     //               {
					...
					//	if (utageResult.HasValue)
					//	{
					//		IsInput = utageResult.Value;
					//	}
					////}
     //           }
     //           else
				...
				case UiStatus.Default:
					...
					{	//入力待ち
						...
						else
						{
                            ...
                            {
								//メッセージ送り
								HHUtageTLStatic.InputHandler?.Invoke();
