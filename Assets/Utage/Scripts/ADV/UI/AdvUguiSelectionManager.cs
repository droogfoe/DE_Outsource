...
namespace Utage
{
	...
	public class AdvUguiSelectionManager : MonoBehaviour
	{
		...
		protected virtual void CreateItems()
		{
            ...
            {
				Button currentBtn = Items[i].gameObject.GetComponent<Button>();
				Navigation nav = currentBtn.navigation;
				nav.mode = Navigation.Mode.Explicit;
                if (i != 0)
                {
					nav.selectOnUp = Items[i - 1].GetComponent<Button>();
                }
                if (i != Items.Count-1)
                {
					nav.selectOnDown = Items[i + 1].GetComponent<Button>();
				}
